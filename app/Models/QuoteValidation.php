...
class QuoteValidation extends Model
{
    ...
    public function approve(int $validatorId): bool
    {
        ...
        if ($saved) {
            ...
            $updates = [
                'valor_convenio' => (float) str_replace([',', '$', ' ', 'MXN'], '', $snapshot['valor_convenio'] ?? 0),
                'valor_compraventa' => (float) str_replace([',', '$', ' ', 'MXN'], '', $snapshot['valor_compraventa'] ?? 0),
                'porcentaje_comision_sin_iva' => (float) str_replace([',', '$', ' ', 'MXN'], '', $snapshot['porcentaje_comision_sin_iva'] ?? 0),
                'precio_promocion' => (float) str_replace([',', '$', ' ', 'MXN'], '', $snapshot['precio_promocion'] ?? 0),
                'monto_credito' => (float) str_replace([',', '$', ' ', 'MXN'], '', $snapshot['monto_credito'] ?? 0),
